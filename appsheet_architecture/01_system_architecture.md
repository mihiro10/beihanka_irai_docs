...
## 2. 状態遷移と4日前正午ルール
依頼データ（Requests）は、製造日までの残日数（4日前正午ルール）によって厳密に操作権限が変化します。
...
stateDiagram-v2
    ...
    state 依頼中 {
        [*] --> 4日前の正午前まで : 自由に修正・削除可能
        [*] --> 4日前の正午以降 : 編集ロック (Read-Only)
    }
    
    4日前の正午前まで --> 4日前の正午以降 : 日時経過 (4日前の正午)
    
    4日前の正午以降 --> 確定 : 米飯課がiPadで承認
    4日前の正午以降 --> 管理者日程調整 : 生産管理による強制上書き
