...
| :--- | :--- | :--- | :--- | :--- |
| **Delete** | Requests | DELETE_RECORD | `=AND(` | 条件に注目。**「すでに確定している」または「4日以内に入っている」場合は削除アイコンが消滅し、押せなくなるガード**が実装されている。 |
...
| **Approve** | Requests | SET_COLUMN_VALUE | `=AND([Status] = "依頼中", [TargetDate] <= (TODAY() + 4))` | **米飯課の確定ボタン:** 4日以内に入ってきた案件のみを「確定」ステータスに変化させるアクション。Deckビューのスワイプに紐付いている。 |
