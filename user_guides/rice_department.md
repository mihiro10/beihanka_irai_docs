...
flowchart TB
    ...
    B -- なし --> G
    E --> G
    G --> H{"依頼された<br>4日前の午前11時以降の作業の実行可否"}
    H -- 問題なし --> I["「確定」押す"]
...
## 2. 「確定（ロック）」ルールについて（4日前午前11時ルール）
誤操作を防ぐため、**「製造日の4日前の午前11時（11:00）」** になるまでは依頼を確定することはできません。
*   **4日前の午前11時前まで:** スケジュールは表示されますが、「確定」ボタンはありません。あくまで「予定」として確認してください。
*   **4日前の午前11時以降:** 画面に **`チェック（確定）`** ボタンが緑色で出現します。人員・材料に問題がなければボタンを押し、スケジュールをロックしてください。
...
*   **登録後の変更・削除:** 「米飯課予定」は確定ルールの対象外のため、アプリのゴミ箱アイコン（Delete）からいつでも自由に削除できます。変更したい場合は一度削除して再登録してください。

![米飯課予定登録画面](images/rice_schedule_block.PNG)

*   **稼働グラフの蓄積ルール（※統合稼働グラフは生産管理部門のみ閲覧可能です）:**
    *   システム上、各時間帯には製造可能な「上限時間（枠）」が設定されています。（※現在の設定例: 昼間は180分、早朝は120分。この上限は時期によって変動します）。
    *   米飯課がこの画面で予定する作業時間（分）を入力すると、**生産管理側のダッシュボード上にある `統合稼働グラフ`** にその時間が積み上がります（ゲージが埋まります）。なお、このグラフは原則として「3週間先」までを表示します。
    *   （※グラフはヒストグラム形式のため、双方いずれの予定も入っていない日はグラフ自体に表示されませんが、カレンダーを見れば空白だと分かります）。
    *   **【例】:** 昼間の枠が合計180分の日に、米飯課が「準備・メンテ：60分」として事前登録した場合、グラフはすでに60分埋まった状態となり、生産管理が後から追加できる依頼は上限までの「残り120分」分となります。（※以下の確認画像は生産管理側の画面です。米飯課のタブレットには表示されません）。
