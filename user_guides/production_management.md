...
flowchart TB
    ...
    Dash -- 「新規依頼」から製造予定を追加（4日前の正午前までの予定） --> Draft["依頼中としてカレンダーに登録される"]
    Dash -- 既存の依頼を変更・キャンセルしたい --> CheckStatus{"現在のステータスは？"}
    CheckStatus -- 4日前の正午以降の確定済み --> Admin["「削除」は不可。<br>米飯課に連絡を入れて必ず<br>「管理者日程調整」ボタンで理由を入力して更新"]
...
## 2. 依頼の修正・キャンセル（4日前正午ルール）
製造日までの「日数」によって、操作方法が厳密に変化します。

### パターンA: 4日前の正午前までの場合（通常キャンセル・再登録）
...
### パターンB: 4日前の正午以降、または「確定済」の場合（管理者強制上書き）
