...
## 2. 状態遷移と4日前午前11時ルール
依頼データ（Requests）は、製造日までの残日数（4日前午前11時ルール）によって厳密に操作権限が変化します。
...
stateDiagram-v2
    ...
    state 依頼中 {
        [*] --> 4日前の午前11時前まで : 自由に修正・削除可能
        [*] --> 4日前の午前11時以降 : 編集ロック (Read-Only)
    }
    
    4日前の午前11時前まで --> 4日前の午前11時以降 : 日時経過 (4日前の午前11時)
    
    4日前の午前11時以降 --> 確定 : 米飯課がiPadで承認
    4日前の午前11時以降 --> 管理者日程調整 : 生産管理による強制上書き
