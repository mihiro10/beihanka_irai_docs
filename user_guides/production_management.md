...
flowchart TB
    ...
    Dash -- 「新規依頼」から製造予定を追加（4日前の午前11時前までの予定） --> Draft["依頼中としてカレンダーに登録される"]
    Dash -- 既存の依頼を変更・キャンセルしたい --> CheckStatus{"現在のステータスは？"}
    CheckStatus -- 4日前の午前11時以降の確定済み --> Admin["「削除」は不可。<br>米飯課に連絡を入れて必ず<br>「管理者日程調整」ボタンで理由を入力して更新"]
...
| 画面名 | 説明 |
| :--- | :--- |
| 📋 米飯課予定登録 | 米飯課が事前に登録した稼働予定（ブロック枠）の一覧を確認できます |
| 📊 生産管理ダッシュボード | カレンダーと統合稼働グラフを並べて表示。新規依頼の作成もここから行います |
| ⚙️ 昼間ダッシュボード | 昼間帯の製造スケジュール一覧。米飯課と同じ画面を閲覧できます |
| 🌅 早朝ダッシュボード | 早朝帯の製造スケジュール一覧。米飯課と同じ画面を閲覧できます |
| 👤 ユーザー管理 | ユーザーの追加・Role変更を行う管理画面（Admin限定） |
| 📦 依頼マスタ | 品目名・ロットサイズ・単位などの製品マスタデータを管理します |
| 📝 確定後の変更履歴 | 「管理者日程調整」で変更された全記録を監査できるログ画面 |
...
![依頼入力画面](images/prod_new_request_details.png)
...
## 2. 依頼の修正・キャンセル（4日前午前11時ルール）
製造日までの「日数」によって、操作方法が厳密に変化します。

### パターンA: 4日前の午前11時前までの場合（通常キャンセル・再登録）
...
### パターンB: 4日前の午前11時以降、または「確定済」の場合（管理者強制上書き）
...
> ⚠️ **【注意】ロット数量を変更した場合は、必ず「目安時間」も合わせて更新してください。** 目安時間を更新しないと、稼働グラフの時間計算が正しく行われません。
