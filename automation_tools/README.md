...
リポジトリ直下にある `appsheet_architecture/02_database_schema.md` と `appsheet_architecture/03_views_actions.md` が自動的に最新の状態に再構築されます。
...
6. ファイル名を必ず **`Application Documentation.html`** （または `app_documentation.html`）とし、**このプロジェクトの一番上のフォルダ（ルートディレクトリ:`beihanka_irai_antigravity/automation_tools/`）**に保存してください。
7. Macのターミナル（Terminal.app）を開き、本リポジトリの直下で以下の実行コマンドを入力します：
   ```bash
   python3 automation_tools/update_app_schema.py
   ```
